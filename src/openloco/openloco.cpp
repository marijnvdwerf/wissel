...
namespace openloco
{
    ...
    static loco_global<uint16_t, 0x00F253A0> _F253A0;
    ...
    void sub_431695()
    {
        ...
        if ((get_screen_flags() & screen_flags::unknown_3) != 0)
        {
            ...
            // run twice as often as _F253A0
            for (auto i = 0; i < _F253A0 * 2; i++)
            {
                sub_428E47();
                WindowManager::dispatchUpdateAll();
            }
    ...
    static void tick()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                _F253A0 = std::max<uint16_t>(1, numUpdates);
                _screen_age = std::min(0xFFFF, (int32_t)_screen_age + _F253A0);
                if (game_speed != 0)
                {
                    numUpdates *= 3;
                    if (game_speed != 1)
