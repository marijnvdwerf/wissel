...
{
public:
    std::vector<wissel::environment::fs::path> fileList;
};
...
static Session* CDECL fn_FindFirstFile(char* lpFileName, FindFileData* out)
{
    ...
    wissel::environment::fs::path path = lpFileName;
    path.remove_filename();

    wissel::environment::fs::directory_iterator iter(path), end;
    ...
    if (wissel::environment::fs::is_directory(data->fileList[0]))
    {
...
void wissel::interop::load_sections()
{
#ifndef _WIN32
...
void wissel::interop::register_hooks()
{
    using namespace wissel::ui::windows;
    ...
    register_hook(
        0x004416B5,
        [](registers& regs) FORCE_ALIGN_ARG_POINTER -> uint8_t {
            using namespace wissel::environment;
    ...
    register_hook(
        0x00498E9B,
        [](registers& regs) FORCE_ALIGN_ARG_POINTER -> uint8_t {
            wissel::ui::windows::sub_498E9B((wissel::ui::window*)regs.esi);
            return 0;
        });
    ...
    register_hook(
        0x004BA8D4,
        [](registers& regs) FORCE_ALIGN_ARG_POINTER -> uint8_t {
            auto v = (wissel::vehicle*)regs.esi;
            v->sub_4BA8D4();
            return 0;
        });
