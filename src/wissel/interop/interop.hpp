...
namespace wissel::utility
{
    ...
    void strcpy_safe(wissel::interop::loco_global<char[TCount], TAddress>& dest, const char* src)
    {
        (void)strlcpy(dest, src, dest.size());
    }

    template<size_t TCount, uintptr_t TAddress>
    void strcat_safe(wissel::interop::loco_global<char[TCount], TAddress>& dest, const char* src)
    {
        (void)strlcat(dest, src, dest.size());
    }

    template<size_t TCount, uintptr_t TAddress, typename... Args>
    int sprintf_safe(wissel::interop::loco_global<char[TCount], TAddress>& dest, const char* fmt, Args&&... args)
    {
