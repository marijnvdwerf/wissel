...
steam_object* wissel::exhaust::object() const
{
    return objectmgr::get<steam_object>(object_id & 0x7F);
}

// 0x0044080C
exhaust* wissel::exhaust::create(loc16 loc, uint8_t type)
{
    if ((uint16_t)loc.x > 12287 || (uint16_t)loc.y > 12287)
        return nullptr;
    auto surface = wissel::map::tilemgr::get(loc.x & 0xFFE0, loc.y & 0xFFE0).surface();
...
// 0x00440BEB
smoke* wissel::smoke::create(loc16 loc)
{
